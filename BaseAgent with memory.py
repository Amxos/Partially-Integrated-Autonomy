# This file contains the BaseAgent class, which is the parent class for all agents in the system. It provides basic functionality for task processing, memory management, and communication with other agents. The BaseAgent class is designed
...
class BaseAgent:
    def __init__(self, role: str, skills: List[str], access_level: int = 1):
        ...
        # Initialize ChromaDB memory (using the ChromaMemory class)
        self.chroma_memory = ChromaMemory(agent_id=self.id)
        logger.info(f"ChromaDB memory initialized for agent {self.id}")
    ...
    def clear_memory(self, older_than: Optional[datetime] = None):
        """Clear agent's memory, optionally only items older than a specific time."""
        with self._lock:
            if older_than is None:
                self.memory.clear()
                self.chroma_memory.clear_memory()  # Also clear ChromaDB memory
                logger.info(f"Memory cleared for agent {self.id}")
                return True

            # Keep items newer than older_than in the deque
            to_keep = deque(maxlen=self.memory.maxlen)
            for item in self.memory:
                if isinstance(item, dict) and "timestamp" in item and datetime.fromisoformat(item["timestamp"]) >= older_than:
                    to_keep.append(item)
            self.memory = to_keep

            # Clear and re-add to ChromaDB (efficient approach for filtering)
            self.chroma_memory.clear_memory()
            for item in to_keep:
              if isinstance(item, str):
                self.chroma_memory.add_to_memory(item) #add it back if its a string
            logger.info(f"Memory cleared for agent {self.id} (items older than {older_than})")
            return True

    def add_to_memory(self, data: str):
        """Add an item to the agent's memory (both deque and ChromaDB)."""
        with self._lock:
            # Add to deque (for quick access to recent items)
            if isinstance(data, dict) and "timestamp" not in data:
                data["timestamp"] = datetime.now().isoformat()
            self.memory.append(data)

            # Add to ChromaDB (for semantic search)
            self.chroma_memory.add_to_memory(data)
            return True


    def query_memory(self, query_text: str, n_results: int = 5) -> List[str]:
        """Query ChromaDB memory for relevant information."""
        return self.chroma_memory.query_memory(query_text, n_results)
